...
function run(choice) {
    ...
    if (i === 1) {
        ...
        if (uchoice === cchoice) {
            ...
            document.getElementById('heading').textContent = "Second innings - You are Batting!🏏";
            userscore = tot;
...
function displayResult(userscore, computerscore) {
    ...
    if (computerscore < userscore) {
        ...
        resultMessage = `Computer wins by ${userscore - computerscore} runs`;
        var audio = new Audio("sounds/loss.mp3");
        audio.play();
    } else if (userscore < computerscore) {
        document.getElementById('heading').textContent = "You win 🎉";
        resultMessage = `Congratulations! You win the match.`;
